...
var constraints: [(CGFloat, CGFloat)] = [(0.001, 2), (2, 6), (6, UITouch().maximumPossibleForce)]
// MARK: - Game State
...
//                print (touch.force)
                ...
                if touch.force == constraints[0].0 {
                    NotificationCenter.default.addObserver(forName: joystickNotificationName, object: nil, queue: OperationQueue.main) { (notification) in
                        guard let userInfo = notification.userInfo else { return }
                        let data = userInfo["data"] as! AnalogJoystickData
...
//
                ...
                if touch.force > 3 {
                    ...
                    NotificationCenter.default.addObserver(forName: joystickNotificationName, object: nil, queue: OperationQueue.main) { (notification) in
                        guard let userInfo = notification.userInfo else { return }
                        let data = userInfo["data"] as! AnalogJoystickData
                        self.hero.position = SCNVector3(self.hero.position.x, self.hero.position.y + Float(data.velocity.y * joystickHeightMultiplier), self.hero.position.z  )
                        
                    }
                    debugPrint("--------in force mode--------")
                } else {
                    debugPrint("--------not in force mode-----")
                }
//rotation
                                if touch.force == constraints[2].0{
                                    NotificationCenter.default.addObserver(forName: joystickNotificationName, object: nil, queue: OperationQueue.main) { (notification) in
                                        guard let userInfo = notification.userInfo else { return }
                                        let data = userInfo["data"] as! AnalogJoystickData
                                        self.hero.position = SCNVector3(self.hero.position.x, self.hero.position.y, self.hero.position.z)
...
//            print(data.description)
    ...
    func moveObj(x: Float, y: Float, z: Float) {
